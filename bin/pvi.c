...
extern int vi_main(int argc, char **argv)
{
	...
	//if (optind >= argc) {	//Èç¹ûÃ»ÓÐ²ÎÊý,Ôò±à¼­¸öÐÂÎÄ¼þ
...
//	case VI_K_END:		// Cursor Key End
	...
	case ':':			// :- the colon mode commands
		...
		if (cnt <= 0)
			break;
		if (strnicmp((char *) p, "quit", cnt) == 0 ||
			strnicmp((char *) p, "q!", cnt) == 0) {	// delete lines
			if (file_modified == TRUE && p[1] != '!') {
				psbs("No write since last change (:quit! overrides)");
			} else {
				editing = 0;
			}
		} else if (strnicmp((char *) p, "write", cnt) == 0 ||
				   strnicmp((char *) p, "wq", cnt) == 0) {
		...
		} else if (strnicmp((char *) p, "file", cnt) == 0 ) {
...
static int file_insert(Byte * fn, Byte * p, int size)
{
	...
	if (fd < 0) {
		...
		fd = open((char *) fn, O_READ);	// try read-only
		if (fd < 0) {
...
static int file_write(Byte * fn, Byte * first, Byte * last)
{
	...
	fd = open((char *) fn, (O_READ | O_CREAT | O_TRUNC), 0664);
	if (fd < 0)
		return (-1);
	cnt = last - first + 1;
...
static void show_status_line(void)
{
	...
	//¼ì²é×Ö·û´®ÊÇ·ñ±ä¸ü£¬ÕâÑù±ÈÖð¸ö±È½Ï¸ü¿ìËÙ£¬µ«ÔÚÄ³Ð©Çé¿öÏÂ£¬»á³öÏÖÎó²î
	//²»¹ýÉÙÁ¿×Ö·ûµÄÇé¿öÏÂÃ»ÎÊÌâ
