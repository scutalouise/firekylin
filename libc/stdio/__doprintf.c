...
static const char * gnum(const char *f, int *ip, va_list *app)
{
	int i, c;
...
static char * _i_compute(printval_u_t val, int base, char *s, int nrdigits)
{
	int c;
...
static char * o_print(va_list *ap, int flags, char *s, char c, int precision,
		int is_signed)
{
	...
	switch (flags & (FL_SHORT | FL_LONG | FL_LONGLONG)) {
		case FL_SHORT:
			if (is_signed) {
				...
				unsigned_val = (unsigned short) va_arg(*ap,
						unsigned);
			}
			break;
		case FL_LONG:
